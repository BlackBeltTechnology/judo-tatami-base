...
/**
 * Work class for Expression validation on ASM using Zeta (native Java) validation rules.
 * This provides an alternative to {@link ExpressionValidationOnAsmWork} which uses Epsilon EVL.
 */
@Slf4j
public class ExpressionValidationOnAsmZetaWork extends AbstractTransformationWork {

    public ExpressionValidationOnAsmZetaWork(TransformationContext transformationContext) {
    ...
    public void execute() throws Exception {
        ...
        // Create AsmModelAdapter for expression validation
        AsmModelAdapter modelAdapter = new AsmModelAdapter(
                asmModel.get().getResourceSet(),
                measureModel.get().getResourceSet()
        );
        ...
        log.info("Expression on ASM Zeta validation completed successfully");
    }
}
