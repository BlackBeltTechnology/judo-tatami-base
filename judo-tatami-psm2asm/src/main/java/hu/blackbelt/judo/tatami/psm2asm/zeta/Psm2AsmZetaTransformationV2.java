 ...
 */
@Slf4j
public class Psm2AsmZetaTransformationV2 {
    ...
    @Builder
    public Psm2AsmZetaTransformationV2(
    ...
    public Map<EObject, List<EObject>> execute() {
        log.info("Starting PSM to ASM Zeta transformation (V2) for model: {}", modelName);
        ...
        log.info("V2: Starting executor.transform()");
        TransformationResult result = executor.transform();
        log.info("V2: Finished executor.transform()");

        // Post-processing: set up inheritance relationships that require all classes to exist
        postProcess(context);

        long duration = System.currentTimeMillis() - startTime;
        log.info("PSM to ASM Zeta transformation (V2) completed in {}ms", duration);
